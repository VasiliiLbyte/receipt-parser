...
2. **Дата**: приведи к формату ГГГГ-ММ-ДД.
   ...
   - Типичные OCR-путаницы в цифрах года: «3»↔«6», «3»↔«8», «5»↔«6», «0»↔«8», «1»↔«7», «4»↔«9»
...
3. **Номер чека**: найди номер чека на самом чеке (обычно в верхней части).
   ...
   - КРИТИЧНО: сохраняй ведущие нули в номере чека (например "0048" не превращать в "48").
   - Номер чека может быть очень коротким: "№ 9", "N 9", "No 9" — это валидный номер "9".
...
4. **ПРАВИЛА распознавания строк чека (перед заполнением `items`)**:
   ...
   - **Двуязычные чеки** (ресторан, отель): название товара может занимать 2 строки —
     английское + русское (или наоборот). Это ОДИН товар! Объедини оба названия
     через пробел в одно поле `name`. Пример:
       `1 Buckwheat tea  1390.00`  ← цена у первой строки
       `Гречишный чай`             ← вторая строка без цены = продолжение названия
     Результат: `"name": "Buckwheat tea Гречишный чай"`, количество 1, цена 1390.
     НЕ создавай отдельную позицию для строки-перевода без цены!
...
8. **ОБЯЗАТЕЛЬНО**: если на чеке есть строка с VAT/НДС/Tax/вкл.НДС и рядом число — ты ОБЯЗАН заполнить поле `total_vat` этим числом. НЕ оставляй `total_vat` равным `null`, если на чеке явно указана сумма НДС.
   Но если на чеке написано «без НДС», «НДС не облагается», «НДС 0%» без числовой суммы — ставь `total_vat: null` или `0`. Не выдумывай число.
   Возможные ставки НДС в РФ (2025-2026): 0%, 5%, 7%, 10%, 20%, 22%.
