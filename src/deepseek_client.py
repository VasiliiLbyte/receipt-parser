...
import json
import re
from .config import DEEPSEEK_API_KEY, API_TIMEOUT
...
- "receipt_number": номер чека, указанный на самом чеке (например, "Чек № 123456"),
- "items": список товаров/услуг, где каждый элемент содержит:
    ...
    - "vat_rate": ставка налога (НДС/VAT/GST/TAX, например "20%", "без НДС", "no VAT"),
    - "vat_amount": сумма налога по строке, только если указана у позиции (число, иначе null)
- "total": общая сумма чека (число),
- "total_vat": итоговая сумма налога с чека (НДС/VAT/GST/TAX в итоговом блоке; число, если нет отдельной суммы — null). Не считать налог самостоятельно.
