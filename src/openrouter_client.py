...
- Также проверь поле "date": сравни дату в JSON с тем, что написано на чеке.
  ...
  Типичные OCR-путаницы в цифрах года: «3»↔«6», «3»↔«8», «5»↔«6», «0»↔«8».
  Если год в JSON выглядит подозрительно старым (2023 вместо 2026) — внимательно
  перечитай цифры года на чеке, особенно последнюю.
...
- Также проверь поле "receipt_number": найди номер чека на изображении и
  ...
  Сохраняй дефисы и буквы внутри номера. Сохраняй ведущие нули (например "0048").
  Номер может быть однозначным: "№ 9", "N 9", "No 9" — это валидный "9".
  Если нечитаемо — оставь как есть.
...
- **ОБЯЗАТЕЛЬНО**: если `total_vat` в JSON равен null, но на чеке видна строка
  VAT/НДС/Tax/вкл.НДС с числом — ЗАПОЛНИ `total_vat` этим числом. Не оставляй null.
